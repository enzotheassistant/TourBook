...
test('finalizeIntakeResult salvages explicit location and DOS lines misplaced into notes', () => {
  const intake = {
    provider: 'test',
    model: 'test',
    rows: [{
      ...
      notes: [
        'City: Toronto',
        'Region: ON',
        'DOS Contact: Jamie Flores 416-555-2000',
        'Buyer asks for settlement after doors.',
      ].join('\n'),
      flags: [],
    }],
  };
  ...
  assert.equal(finalized.rows[0].dos_name, 'Jamie Flores');
  assert.equal(finalized.rows[0].dos_phone, '416-555-2000');
  assert.equal(finalized.rows[0].notes, 'Buyer asks for settlement after doors.');
});
