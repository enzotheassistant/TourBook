...
test('finalizeIntakeResult backfills single-row venue and partial contact data from source text', () => {
  const intake = {
    provider: 'test',
    model: 'test',
    rows: [{
...
test('finalizeIntakeResult flags ambiguous contacts and duplicate dates in multi-row imports without copying shared data blindly', () => {
  ...
  assert.ok(!finalized.warnings.some((warning) => warning.includes('multiple possible rows or people')));
